...
class Viewer(QWidget):
    ...
    def makePixmap(self, page_num: int, for_printing=False) -> QPixmap:
        ...
        # there was a reason to specify the Format but can't remember atm
...
class PDF_Exporter(QThread):
    ...
    def __init__(self):
        ...
        d = time.localtime()
        self.datejour = time.strftime('%d/%m/%Y', d)
    ...
    def run(self):
        ...
        """
        self.running = True
        self.multi_page_export(self.settings['book'], self.settings['viewer'], self.settings['topics'])
    ...
    def multi_page_export(self, book: dict, pdf_viewer: Viewer, topics: dict):
        """
        ...
        clean_book = [a for a in sorted(book.keys()) if PDF_Exporter.eval_doc(book[a])]
        ...
        for page_pdf in clean_book:
            ...
            # this means we're on the first page, the one with the PDF printed and half page filled with text
            if self.page >= 1:
                # generating a pixmap for the current page, low or highres
                pixmap = pdf_viewer.makePixmap(page_pdf, self.settings['hq'])

                # scaling the pixmap to fit in the page
                pixmap = pixmap.scaledToWidth(int(self.currentRect.width() / 2 + self.mm2px(3)))

                # drawing the pixmap in place
                self.painter.drawPixmap(self.currentRect.topRight() - QPointF(self.currentRect.width() / 2, 0), pixmap)
                self.painter.restore()
                self.painter.save()

                # setting the rect to draw the topics part
                rect = QRectF(QPointF(0, 0),
                              QSizeF(self.rect.size().width() // 2, self.rect.size().height() // 2))
                delta = QPoint(int(rect.width()) + self.mm2px(10), pixmap.height() + self.mm2px(10))
                ...
                topic_doc.setHtml(self.build_topics(topics['contexts']))
                topic_doc.drawContents(self.painter, rect)
            ...
            if page_pdf in topics['contexts']:
                for topic in topics['contexts'][page_pdf]:
        ...
        self.paint_tot(printer, self.format_tot(topics['domains']))
    ...
    def pdf_snapshot(self, book: dict):
        """
        ...
        for page_pdf in clean_book:
            ...
            self.currentRect = QRectF(QRect(QPoint(0, 0), self.rect.size()))
            head, body = self.post_treatment(book[page_pdf])
    ...
    def format_tot(self, topics_domains: dict) -> dict:
        """
        ...
        domains_set = {
            'prophet': {'title': "Index des Prophètes عليهم الصلاة و السلام", 'topics': set(), 'doc': "<table>"},
            'person': {'title': "Index des personnes", 'topics': set(), 'doc': "<table>"},
            'place': {'title': "Index des lieux", 'topics': set(), 'doc': "<table>"},
            'theme': {'title': "Index des thèmes", 'topics': set(), 'doc': "<table>"},
            'fiqh': {'title': "Index des questions jurisprudencielles", 'topics': set(), 'doc': "<table>"},
            'nahw': {'title': "Index des termes grammaticaux", 'topics': set(), 'doc': "<table>"},
            'lugha': {'title': "Index linguistique", 'topics': set(), 'doc': "<table>"}
        }

        # TODO : correction orthographique classé ??

        for topic in topics_domains:
            domains_set[topics_domains[topic]]['topics'].add(topic)
        ...
        for domain in domains_set:
            ...
            for topic in sorted(domains_set[domain]['topics'], key=lambda x:x.lower()):
                try:
                    nice_topic = topic[0].upper() + topic[1:]
                    pages = ", ".join(map(str, sorted(self.topics[topic])))
                    doc += '<tr>' \
                           f'<td style="{PDF_Exporter.linked_title_cell_css}{PDF_Exporter.title_cell_css[2]}">{nice_topic}</td>' \
                           f'<td style="{PDF_Exporter.linked_page_cell_css}{PDF_Exporter.multipage_cell_css}" width="15%">' \
    ...
    def build_topics(self, topics: dict) -> str:
        page = self.page + 1

        # trying to nicely format the topics for the given page
        try:
            # abort if len == 0
            assert len(topics[page]) > 0
    ...
    def post_treatment(self, doc: str) -> (str, str):
        ...
        doc_content = re.sub(r' ﴾', r'﴾', doc_content, re.MULTILINE)
        doc_content, nb = re.subn(r'([١٢٣٤٥٦٧٨٩٠])\)</span></p>\n.*?-qt-paragraph-type:empty;.*?\n',
                                  r'\1)</span></p>', doc_content, re.MULTILINE | re.DOTALL)
        ...
        # TODO:mettre en violet la séparation des verset ۝
        ...
        try:
            ...
            translation = re.sub(r'(<p.*?style=")(.*?)(margin-top:\dpx;)(.*?">)(\d+)\. ?(.*?)</p>',
                                 r"""\1\2margin-top:20px;\4<span style=" font-family:'AGA Arabesque'; font-size:10pt; color:#9622d3;">{</span><span style="color:#9622d3;"> \5 </span><span style=" font-family:'AGA Arabesque'; font-size:10pt; color:#9622d3;">}</span> <span style=" font-family:'Microsoft Uighur'; font-size:15pt; color:#267dff; font-weight:600;">\6</span></p>""",
                                 translation)
    ...
    def paint_body_text(self, head: str, text: str, printer: QtPrintSupport.QPrinter):
        ...
        if self.page >= 1:
            # we draws the formatted head content
